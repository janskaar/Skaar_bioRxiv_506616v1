...
def eval_dgp_likelihood(sample, y0, model, batch_size=100):
    num = len(sample)
    if len(y0.shape) == 1:
        y0 = y0.unsqueeze(0)
    y0 = y0.tile((num, 1))

    num_batches = int(len(sample) / batch_size) if len(sample) % batch_size == 0 else int(len(sample) / batch_size) + 1
    log_probs = []
    sample = sample.to(DEVICE)
    for i in range(num_batches):
        # reshape to (num, 1, output_features), so we
        # compute distribution independently for each example
        x_batch = sample[i*batch_size:(i+1)*batch_size]
        y_batch = y0[i*batch_size:(i+1)*batch_size]

        dist = model.likelihood(model.memory_efficient_call(x_batch[:,None,:].to(DEVICE)))
        log_probs_ = dist.log_prob(y_batch[None,:,None,:].to(DEVICE))
        log_probs_ = torch.logsumexp(log_probs_, 0) - torch.log(torch.tensor(log_probs_.shape[0]))
        log_probs.append(log_probs_)
    return torch.cat(log_probs).cpu()

dgp_likelihood = DeepLMC(10, num_latent_gps=50, num_hidden_dgp_dims=20, num_tasks=258).to(DEVICE)
state_dict = torch.load("dgp_20_50_0001_13.model", map_location=torch.device(DEVICE))
dgp_likelihood.load_state_dict(state_dict)
dgp_likelihood.eval();

eval_dgp_likelihood = partial(eval_dgp_likelihood, model=dgp_likelihood, batch_size=50)

num_burn_in = 12000
num_steps = 28000
num_chains = 10
for ind in range(32,33):
    savedir = os.path.join('mcmc_posterior_samples_dgp', f'{ind:04d}')
    os.mkdir(savedir)
    ...
    with torch.no_grad():
        ...
        mh =  MH(eval_dgp_likelihood, y0, num_chains=num_chains)
