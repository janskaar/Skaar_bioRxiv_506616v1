...
psds = np.load(os.path.join(dpath, '10p_lfp_psd_nosync.npy'))
labels = np.load(os.path.join(dpath, '10p_labels_nosync.npy')).astype(np.float64)
## In order to save computation time and memory, use two channels
psds = np.concatenate([psds[:,0,:], psds[:,4,:]], axis=1)
...
## Adjust likelihood to corresponding two channels used
mask = torch.zeros(6*129, dtype=bool)
mask[:129] = True
mask[4*129:5*129] = True
model.last_layer.variational_strategy.lmc_coefficients.data = model.last_layer.variational_strategy.lmc_coefficients.data[:,mask]
model.likelihood.raw_task_noises.data = model.likelihood.raw_task_noises.data[mask]
...
class MH:
    def __init__(self, model, y0, cov = None, sigma = 0.5, num_chains=5):
        ...
        self.num_accepted = np.zeros(self.num_chains, dtype=np.float32)             # keep track of number of accepted samples for adjusting proposal scale
        self.sigma = torch.ones(self.num_chains) * sigma                            # scaling factor for covariance for proposal
        self.y0 = y0
    ...
    def burn_in(self, pos, num_steps):
        ...
        for i in range(num_steps):
            ...
            ## Exponential adjustment of sigma
            if i > 20:
