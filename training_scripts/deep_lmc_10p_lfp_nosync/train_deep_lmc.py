...
psds = np.load(os.path.join(data_path, '10p_lfp_psd_nosync.npy')).astype(np.float32)
labels = np.load(os.path.join(data_path, '10p_labels_nosync.npy')).astype(np.float32)
psds = np.concatenate([psds[:,i,:] for i in range(6)], axis=1)
...
X_test = labels[n_train:].copy()
Y_test = np.log10(psds[n_train:].copy())
