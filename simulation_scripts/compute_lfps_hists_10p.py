...
ehists = np.array(ehists)
ihists = np.array(ihists)
lfps = np.array(lfps)
ps = []
for param in params:
...
# Remove simulations in which the activity is strongly synchronous
args = ((ehists > 800).sum(axis=1) > 150) & ((ehists < 20).sum(axis=1) > 500)

# Compute PSDS, discard first 501 time due to start-up effects
fs, lfppsds1 = welch(lfps[~args,:,501:][:1900])
fs, lfppsds2 = welch(lfps[~args,:,501:][1900:])
lfppsds = np.concatenate((lfppsds1, lfppsds2), axis=0)
fs, histpsds = welch(hists[~args,:,501:])
ps = ps[~args]

np.save('10p_hist_psd_nosync.npy', histpsds)
np.save('10p_lfp_psd_nosync.npy', lfppsds)
np.save('10p_labels_nosync.npy', ps)
